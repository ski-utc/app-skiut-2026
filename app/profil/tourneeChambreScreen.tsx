...
export default function TourneeChambreScreen() {
  ...
  const fetchTourData = useCallback(async () => {
    ...
    try {
      ...
      if (isSuccessResponse(response)) {
        ...
        if (
          response.message &&
          response.message.includes("membres de l'association")
        ) {
          setError(
            "Cette fonctionnalité est réservée aux membres de l'association.",
          );
        } else {
          setTourData(null);
          setVisits([]);
  ...
  const markAsVisited = async (visit: Visit) => {
    try {
      const response = await apiPost(
        `room-tours/visits/${visit.id}/mark-visited`,
        {},
      );
      ...
      [
        { text: 'Non', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await apiPost(
                `room-tours/visits/${visit.id}/unmark-visited`,
                {},
              );
  ...
  const saveOrder = async () => {
    ...
    try {
      ...
      const response = await apiPost('room-tours/my-tour/reorder', {
            ...
            <View style={styles.roomInfo}>
              <View style={styles.roomHeaderRow}>
                <Text
                  style={[
                    styles.roomNumber,
                    item.visited && styles.textVisited,
                  ]}
                >
              ...
              {item.room_info.room_name && (
                <Text
                  style={[styles.roomName, item.visited && styles.textVisited]}
                >
                  {item.room_info.room_name}
                </Text>
              )}
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.binomeCard}>
          <View style={styles.binomeHeader}>
            <View style={styles.binomeTitleRow}>
              ...
              <Text style={styles.binomeTitle}>
                Avec {tourData.binome.teammate.name}
              </Text>
            </View>
            <Text style={styles.binomeStats}>
              {tourData.binome.stats.visited_rooms}/
              {tourData.binome.stats.total_rooms}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.titleSection}>
          ...
          <Text style={styles.dragHint}>
            Maintenez l'icône <GripVertical size={12} color={Colors.muted} />{' '}
            pour réorganiser
          </Text>
        </View>
