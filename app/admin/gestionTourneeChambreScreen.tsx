...
export default function GestionTourneeChambreScreen() {
  ...
  const updateBinome = <K extends keyof Binome>(
    index: number,
    field: K,
    value: Binome[K],
  ) => {
    ...
    for (const binome of binomes) {
      if (
        ...
        Toast.show({
          type: 'error',
          text1: 'Erreur',
          text2: `Le ${binome.name} est incomplet.`,
        });
        return;
      }
    }
    ...
    try {
      ...
      if (isSuccessResponse(response)) {
        ...
        Toast.show({
          type: 'info',
          text1: 'Sauvegardé',
          text2: 'Création en attente de connexion.',
        });
        setShowCreateModal(false);
        resetForm();
        fetchData();
            ...
            onPress: async () => {
              try {
                const response = await apiPost(
                  `admin/room-tours/${tour.id}/toggle`,
                  {
      ...
      <View style={styles.tourCard}>
        ...
        <View style={styles.cardStats}>
          ...
          <View style={styles.statRow}>
            ...
            <Text style={styles.statText}>
              {item.progress_percentage}% complété
            </Text>
          </View>
        </View>
    ...
    <SafeAreaView style={styles.container} edges={['bottom', 'left', 'right']}>
      ...
      <View style={styles.heroSection}>
        ...
        <Text style={styles.heroSubtitle}>
          {tours.length} tournée{tours.length !== 1 ? 's' : ''} planifiée
          {tours.length !== 1 ? 's' : ''}
        </Text>
      </View>
              ...
              <View style={styles.pickerModalContent}>
                ...
                <ScrollView style={styles.pickerModalList}>
                  {members.map((member) => {
                    ...
                    const isSelected =
                      currentBinome?.member_ids.includes(member.id) || false;
                    const canSelect =
                      currentBinome?.member_ids.length < 2 || isSelected;
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          !canSelect && styles.pickerModalItemDisabled,
                        ]}
                        onPress={() => {
                          if (!canSelect && !isSelected) return;
                          const newBinomes = [...binomes];
                          if (isSelected) {
                            newBinomes[binomeIndex].member_ids = newBinomes[
                              binomeIndex
                            ].member_ids.filter((id) => id !== member.id);
                          } else {
                            if (newBinomes[binomeIndex].member_ids.length < 2) {
                              newBinomes[binomeIndex].member_ids.push(
                                member.id,
                              );
                            }
                          }
              ...
              <View style={styles.pickerModalContent}>
                ...
                <ScrollView style={styles.pickerModalList}>
                  {rooms.map((room) => {
                    ...
                    const currentBinome = binomes[binomeIndex];
                    const isSelected =
                      currentBinome?.assigned_rooms.includes(room.room_id) ||
                      false;
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          const newBinomes = [...binomes];
                          if (isSelected) {
                            newBinomes[binomeIndex].assigned_rooms = newBinomes[
                              binomeIndex
                            ].assigned_rooms.filter(
                              (id) => id !== room.room_id,
                            );
                          } else {
                            newBinomes[binomeIndex].assigned_rooms.push(
                              room.room_id,
                            );
                          }
                          setBinomes(newBinomes);
