...
export default function GestionPermanencesScreen() {
  ...
  const [editingPermanence, setEditingPermanence] = useState<Permanence | null>(
    null,
  );
  ...
  const [formResponsibleId, setFormResponsibleId] = useState<number | null>(
    null,
  );
  ...
  const submitPermanence = async () => {
    ...
    if (formStartDate <= new Date()) {
      Toast.show({
        type: 'error',
        text1: 'Erreur',
        text2: 'La date de début doit être dans le futur.',
      });
      return;
    }
    if (formStartDate >= formEndDate) {
      Toast.show({
        type: 'error',
        text1: 'Erreur',
        text2: "L'heure de fin doit être postérieure au début.",
      });
      return;
    }
    ...
    try {
      ...
      const endpoint = editingPermanence
        ? `admin/permanences/${editingPermanence.id}`
        : 'admin/permanences';
          ...
          onPress: async () => {
            try {
              const response = await apiDelete(
                `admin/permanences/${permanence.id}`,
              );
              ...
              if (isSuccessResponse(response)) {
                Toast.show({
                  type: 'success',
                  text1: 'Succès',
                  text2: 'Permanence supprimée.',
                });
                setPermanences((prev) =>
                  prev.filter((p) => p.id !== permanence.id),
                );
              } else if (isPendingResponse(response)) {
                Toast.show({
                  type: 'info',
                  text1: 'Sauvegardé',
                  text2: 'Suppression en attente de connexion.',
                });
                setPermanences((prev) =>
                  prev.filter((p) => p.id !== permanence.id),
                );
          ...
          onPress: async () => {
            try {
              const response = await apiPost(
                'admin/permanences/send-reminders',
                {},
              );
              if (isSuccessResponse(response)) {
                Toast.show({
    ...
    <SafeAreaView style={styles.container} edges={['bottom', 'left', 'right']}>
      ...
      <View style={styles.heroSection}>
        ...
        <Text style={styles.heroSubtitle}>
          {filteredPermanences.length} permanence
          {filteredPermanences.length !== 1 ? 's' : ''} à venir
        </Text>
      </View>
                ...
                >
                  <View style={styles.selectionHeaderLeft}>
                    ...
                    <Text style={styles.selectionTitle}>
                      Responsable *{' '}
                      {formResponsibleId &&
                        `(${members.find((m) => m.id === formResponsibleId)?.name})`}
                    </Text>
                  </View>
                  {showMembersPicker ? (
                ...
                <View style={styles.dateTimeRow}>
                    ...
                    >
                      ...
                      <View style={styles.dateTimeTextContainer}>
                        <Text style={styles.dateTimeText}>
                          {formStartDate.toLocaleDateString('fr-FR', {
                            day: '2-digit',
                            month: '2-digit',
                          })}
                        </Text>
                        <Text style={styles.dateTimeTime}>
                          {formStartDate.toLocaleTimeString('fr-FR', {
                            hour: '2-digit',
                            minute: '2-digit',
                          })}
                        </Text>
                  ...
                  <View style={styles.dateTimeItem}>
                    ...
                    >
                      ...
                      <View style={styles.dateTimeTextContainer}>
                        <Text style={styles.dateTimeText}>
                          {formEndDate.toLocaleDateString('fr-FR', {
                            day: '2-digit',
                            month: '2-digit',
                          })}
                        </Text>
                        <Text style={styles.dateTimeTime}>
                          {formEndDate.toLocaleTimeString('fr-FR', {
                            hour: '2-digit',
                            minute: '2-digit',
                          })}
                        </Text>
            ...
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity
                ...
                style={[
                  ...
                  (!formName.trim() || !formResponsibleId) &&
                    styles.modalSaveButtonDisabled,
                ]}
                disabled={
                  formSubmitting || !formName.trim() || !formResponsibleId
                }
              >
                {formSubmitting ? (
