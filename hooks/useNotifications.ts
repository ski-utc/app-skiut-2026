...
export const useNotifications = () => {
  ...
  useEffect(() => {
    const loadInitialState = async () => {
      try {
        const notificationsRegistered = await AsyncStorage.getItem(
          'notificationsRegistered',
        );
        if (notificationsRegistered === 'true') {
          setIsInitialized(true);
          console.log(
            '[useNotifications] Notifications déjà enregistrées - isInitialized = true',
          );
        } else {
        ...
        setIsInitialized(false);
        console.error(
          "[useNotifications] Erreur lors du chargement de l'état initial:",
          error,
        );
      }
    };
  ...
  const initializeNotifications = useCallback(async () => {
    ...
    if (notificationsRegistered === 'true') {
      console.log(
        '[useNotifications] Notifications déjà enregistrées - pas de ré-initialisation',
      );
      setIsInitialized(true);
      return;
    }
    ...
    try {
      ...
      console.log(
        '[useNotifications] Initialisation terminée, success:',
        success,
      );
      ...
      console.error(
        '[useNotifications] Erreur initialisation notifications:',
        error,
      );
  ...
  useEffect(() => {
    ...
    if (user && !isInitialized) {
      timeoutId = setTimeout(() => {
        if (!isInitialized) {
          console.warn(
            "Timeout initialisation notifications - l'app continue sans notifications",
          );
          setIsInitialized(true);
        }
      }, 10000);
