...
const PodiumPlace: React.FC<PodiumPlaceProps> = ({
  ...
  return (
    <View style={podiumStyles.placeContainer}>
      ...
      <Text style={podiumStyles.roomText} numberOfLines={1}>
        {fullName}
      </Text>
...
export default function PerformancesScreen() {
  ...
  const fetchPerformances = useCallback(async () => {
    ...
    try {
      const response = await apiGet<PerformanceResponse>(
        `classement-performances?type=${rankingType}`,
      );
  ...
  const getValueDisplay = (item: PerformanceData) => {
    switch (rankingType) {
      case 'speed':
        return `${Number(item.max_speed).toFixed(1)} km/h`;
      case 'distance':
        return `${Number(item.total_distance || 0).toFixed(2)} km`;
      case 'duration':
        return `${Math.floor(Number(item.duration || 0) / 3600)}h${Math.floor((Number(item.duration || 0) % 3600) / 60)}min`;
      default:
        return '';
    }
  };
        ...
        <View style={styles.dropdownContainer}>
          ...
          >
            ...
            <ChevronDown
              ...
              style={
                showRankingMenu ? styles.chevronIconRotated : styles.chevronIcon
              }
            />
          </TouchableOpacity>
