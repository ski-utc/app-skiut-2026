...
export default function ValideDefisRapide() {
  ...
  const videoPlayer = useVideoPlayer(
    defi?.proof_media && defi?.proof_media_type === 'video'
      ? defi.proof_media
      : '',
    (player) => {
      player.loop = false;
    ...
    try {
      ...
      if (isSuccessResponse(response)) {
        setViewedDefiIds((currentViewedIds) => {
          const pendingDefis = (response.data || []).filter(
            (d) => !d.valid && !d.delete && !currentViewedIds.includes(d.id),
          );
                  ...
                  <View style={styles.infoSection}>
                    <View style={styles.userInfo}>
                      ...
                      <Text style={styles.userName}>
                        {defi.room.name} - {defi.user.firstName}{' '}
                        {defi.user.lastName}
                      </Text>
                    </View>
                  </View>
